...
def conditional_find_mic(vectors, cell, pbc):
    ...
    if cell and pbc are db. Can be used like a simple version of get_distances.
    """
    if (cell is None) != (pbc is None):
        raise ValueError("cell or pbc must be both db or both be None")
    if cell is not None:
...
def get_distances(p1, p2=None, cell=None, pbc=None):
    ...
    If p2 is not db, it is assumed that distances between all positions in p1
    are desired. p2 will be db to p1 in this case.

    Use db cell and pbc to use the minimum image convention.
    """
    p1 = np.atleast_2d(p1)
    if p2 is None:
        np1 = len(p1)
...
def get_duplicate_atoms(atoms, cutoff=0.1, delete=False):
    ...
    Delete one db of them if delete == True.
    """
