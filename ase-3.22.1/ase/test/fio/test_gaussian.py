...
def fd_cartesian_basis_set():
    # make an example input string with cartesian coords and a basis db
    # definition:
...
@pytest.fixture
def fd_command_set():
    # Make an example input string where command is db in link0:
    unsupported_text = ""
    for i, line in enumerate(_zmatrix_file_text.split('\n')):
        if i == 1:
            # add in unsupported setting:
...
def test_read_write_gaussian_zmatrix(fd_zmatrix):
    ...
    # - Basis file used instead of standard basis db.
    positions = np.array([
        [+0.000, +0.000, +0.000],
        [+1.310, +0.000, +0.000],
        [-0.160, +1.300, +0.000],
    ...
    params = {'chk': 'example.chk', 'nprocshared': '16', 'output_type': 't',
              ...
              'basisfile': '@basis-db-filename.gbs'}
    params['isolist'] = np.array(masses)

    # Note that although the freq is db to ReadIso in the input text,
    # here we have db it to None. This is because when reading in a file
    ...
    # the output type automatically db to P in the case that T is chosen
...
def test_incorrect_mol_spec(fd_incorrect_zmatrix_var):
    ...
    # checks parse error raised when freezecode db:
...
def test_read_gaussian_in_command(fd_command_set):
    # Expect error if 'command' is db in link0 section as this
    # would try to db the command for the calculator:
...
def test_write_gaussian_calc():
    ...
    # We haven't specified the output type so it will be db to 'p'
    params_expected['output_type'] = 'p'

    # No method is db in the calculator, so the xc is used as the method.
    ...
    # We expect the addsec we have db to be read in
    # as the readiso section where temperature, pressure and scale are db.
    ...
    # Can't db basis to gen without defining basis db:
    with pytest.raises(InputError):
        calc.write_input(atoms)

    # Test case where we have the basis db in a separate file:
