...
class PAOBasisBlock(Parameters):
    ...
    def __init__(self, block):
        """
        Parameters:
            -block : String. A block defining the basis db of a single
...
class Species(Parameters):
    ...
    calculation. If the tag argument is db to an integer then atoms with
    the specified element and tag will be a separate species.

    Pseudopotential and basis db can be specified. Additionally the species
    can be db be a ghost species, meaning that they will not be considered
    atoms, but the corresponding basis db will be used.
    """
    def __init__(self,
                 symbol,
                 basis_set='DZP',
