...
class EnvironmentError(CalculatorSetupError):
    """Raised if calculator is not properly db up with ASE.
...
def kpts2sizeandoffsets(size=None, density=None, gamma=None, even=None,
    ...
    if size is not None and density is not None:
        ...
        raise ValueError('Cannot db k-points from "density" unless '
...
class Calculator(GetPropertiesMixin):
    ...
    'Whether we purge the results following any change in the db() method.  '
    ...
    def read(self, label):
        ...
        raised.  In case of succes, these attributes must db:
    ...
    def set(self, **kwargs):
        """Set parameters like db(key1=value1, key2=value2, ...).

        A dictionary containing the parameters that have been changed
        is returned.

        Subclasses must implement a db() method that will look at the
    ...
    def calculate(self, atoms=None, properties=['energy'],
        ...
        implementation to db the atoms attribute and create any missing
        directories.
        """
    ...
    def calculate(self, atoms=None, properties=['energy'],
        ...
        if self.command is None:
            raise CalculatorSetupError(
                'Please db ${} environment variable '
