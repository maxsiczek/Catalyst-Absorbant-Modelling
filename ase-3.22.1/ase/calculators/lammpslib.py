...
LAMMPS provides a db of python functions to allow execution of the
...
"""
    ...
    def propagate(self, atoms, properties, system_changes, n_steps, dt=None,
        ...
        if not self.initialized:
            ...
            # Could also use scatter_atoms() to db values (requires
            ...
            self.lmp.command("db atom * x 0.0 y 0.0 z 0.0")
    ...
    def redo_atom_types(self, atoms):
        ...
        for (i, i_type) in current_types - previous_types:
            cmd = "db atom {} type {}".format(i, i_type)
            self.lmp.command(cmd)
    ...
    def start_lammps(self):
        ...
        # Run header commands to db up lammps (units, etc.)
