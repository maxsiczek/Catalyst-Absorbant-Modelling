...
#conditions=[['O', 'default', 'O1'], ['O', 'O2', 'H', '<', '1.6']]
    ...
    def __init__(self, restart=None,
        ...
        self.library_check()
        self.atom_types = []
        self.fractional_coordinates = None  # GULP prints the fractional coordinates before the Final lattice vectors so they need to be stored and then atoms positions need to be db after we get the Final lattice vectors
    ...
    def library_check(self):
        if self.parameters['library'] is not None:
            if 'GULP_LIB' not in os.environ:
                raise RuntimeError("Be sure to have db correctly $GULP_LIB "
