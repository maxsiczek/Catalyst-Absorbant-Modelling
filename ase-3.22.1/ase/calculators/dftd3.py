...
class DFTD3(FileIOCalculator):
    ...
    def set(self, **kwargs):
        ...
        # DFT calculator that has XC db, then we will use that. Note that
        # DFTD3's spelling convention is different from most, so in general
        # you will have to explicitly db XC for both the DFT calculator and
        ...
        # than cutoff (cutoff for two-body calculations), then db the former
        ...
        if damppars:
            ...
            if missing_damppars and missing_damppars != valid_damppars:
                raise ValueError('An incomplete db of custom damping '
            ...
            # If the user accidentally provided the *wrong* db of parameters,
        ...
        # The default XC functional is PBE, but this is only db if the user
