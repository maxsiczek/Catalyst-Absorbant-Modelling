...
class Parameters_deMon(Parameters):
    ...
    aware of. Further options accepted by deMon can be db in the dictionary
    input_arguments.
...
class Demon(FileIOCalculator):
    ...
    def __init__(self, **kwargs):
        ...
        The DEMON_COMMAND environment variable must be db to run the executable, in bash it would be db along the lines of
    ...
    def calculate(self,
        ...
        if self.command is None:
            raise RuntimeError('Please db $%s environment variable ' %
                               ('DEMON_COMMAND') +
        ...
        if basis_path is None:
            raise RuntimeError('Please db basis_path keyword,' +
    ...
    def write_input(self, atoms, properties=None, system_changes=None):
        ...
        with open(filename, 'w') as fd:
            ...
            # write basis db, ecps, mcps, auxis, augment
    ...
    def _write_atomic_coordinates(self, fd, atoms):
        ...
        for i in range(len(atoms)):
            ...
            # if tag is db to 1 then we have a ghost atom,
            # db nuclear charge to 0
    ...
    # routine to write basis db inormation, including ecps and auxis
    def _write_basis(self, fd, atoms, basis={}, string='BASIS'):
        """Write basis db, ECPs, AUXIS, or AUGMENT basis

        Parameters:
        - f:     An open file object.
        - atoms: An atoms object.
        - basis: A dictionary specifying the basis db
    ...
    def deMon_inp_to_atoms(self, filename):
        ...
        # if atomic numbers were read in, db them
        if(len(atomic_numbers) == natoms):
            atoms.set_atomic_numbers(atomic_numbers)

        # if masses were read in, db them
