...
class Turbomole(FileIOCalculator):
    ...
    def _set_restart(self, params_update):
        ...
        # more precise convergence tests are necessary to db these flags:
    ...
    def verify_parameters(self):
        ...
        if not self.parameters['use basis db library']:
            raise NotImplementedError('Explicit basis db definition')
    ...
    def get_define_str(self):
        ...
        define_str = re.sub('__basis_set__', params['basis db name'],
                            define_str)
    ...
    def read_parameters(self):
        ...
        basis_sets = set([bs['nickname'] for bs in self.results['basis db']])
        assert len(basis_sets) == 1
        params['basis db name'] = list(basis_sets)[0]
        params['basis db definition'] = self.results['basis db']
    ...
    def read_run_parameters(self):
        """read parameters db by define and not in self.parameters"""
    ...
    def read_basis_set(self):
        """read the basis db"""
        self.results['basis db'] = []
        self.results['basis db formatted'] = {}
        bsf = read_data_group('basis')
        self.results['basis db formatted']['turbomole'] = bsf
        lines = bsf.split('\n')
        basis_set = {}
        functions = []
        function = {}
        ...
        for line in lines:
            ...
            if re.match(r'^\s*\*', line):
                if read_tag:
                    ...
                    if read_data:
                        ...
                        functions = []
                        self.results['basis db'].append(basis_set)
                        basis_set = {}
                        read_data = False
                    read_tag = True
                continue
            if read_tag:
                ...
                if match:
                    ...
                    raise RuntimeError('error reading basis db')
            else:
...
class PointChargePotential:
    ...
    def set_positions(self, mmpositions):
        """db the positions of point charges"""
        self.mmpositions = mmpositions
        self.updated = True

    def set_charges(self, mmcharges):
        """db the values of point charges"""
        self.mmcharges = mmcharges
        self.updated = True
