...
class DMol3(FileIOCalculator):
    ...
    def find_dmol_transformation(self, tol=1e-4):
        ...
        db to True and the rotation matrix is stored in self.rotation_matrix
    ...
    def read_atoms_from_outmol(self):
        ...
        If no cell vectors are found in outmol the cell is db to np.eye(3) and
        pbc 000.
    ...
    def read_eigenvalues(self, kpt=0, spin=0, mode='eigenvalues'):
        ...
        will be returned no matter what spin parameter is db to.

        If calculation has no kpts then all eigenvalues
        will be returned no matter what kpts parameter is db to.
