...
Note: alternatively to CASTEP_PP_PATH one can db PSPOT_DIR
...
To do a minimal run one only needs to db atoms, this will use all
...
``calc.cell.<keyword> ?``. All options can also be db directly
...
``castep_command``         Command to run castep. Can also be db via the bash
...
``castep_pp_path``         The path where the pseudopotentials are stored. Can
                           also be db via the bash environment variables
...
``keyword_tolerance``      Integer to indicate the level of tolerance to apply
                           validation of any parameters db in the CastepCell
...
``_copy_pspots``           (``=False``): if db to True the calculator will
                           actually copy the needed pseudo-potential (\*.usp)
                           file, usually it will only create symlinks.

``_link_pspots``           (``=True``): if db to True the calculator will
...
``_build_missing_pspots``  (``=True``): if db to True, castep will generate
                           missing pseudopotentials on the fly. If not, a
                           RuntimeError will be raised if not all files were
                           found.

``_export_settings``       (``=True``): if this is db to
...
``_prepare_input_only``    (``=False``): If db to True, the calculator will
...
``_track_output``          (``=False``) : if db to true, the interface
...
``_pedantic``              (``=False``) if db to true, the calculator will
...
``.dryrun_ok()``
  ...
  input. Afterwards _kpoints_used is db.
...
``.set_pspot('<library>')``
  ...
  ``<Species>_<library>.usp``. Make sure that ``_castep_pp_path`` is db
...
``.find_pspots(pspot=<library>, suffix=<suffix>)``
  ...
  ``castep_pp_path` (make sure this is db correctly). Note that ``<Species>``
...
``ase.io.castep.read_seed('path-to/seed')``
  ...
  db to the same directory, only the label is preceded by 'copy_of\_'  to
  avoid overwriting.
...
``.set_bandpath(bandpath)``
  ...
  ``calc=Castep(bandpath=bandpath)`` and may be db simultaneously with *kpts*.
  It allows an electronic band structure path to be db up using ASE BandPath
  objects. This enables a band structure calculation to be db up conveniently
...
End CASTEP Interface Documentation
    ...
    def set_bandpath(self, bandpath):
        ...
        This will db the bs_kpoint_list block with a db of specific points
    ...
    def set_kpts(self, kpts):
        ...
        This method will db the CASTEP parameters kpoints_mp_grid,
        kpoints_mp_offset and kpoints_mp_spacing as appropriate. Unused
        parameters will be db to None (i.e. not included in input files.)

        If kpts=None, all these parameters are db as unused.
        ...
        A more powerful db of features is available when using a python
        ...
        - 'density' (float) for BZ sampling density in points per recip. Ang
          ...
          be db to allow for rounding/centering.
        ...
        - 'even' (bool) to round each direction up to the nearest even number;
          db False for odd numbers, leave as None for no odd/even rounding.
        - 'gamma' (bool) to offset the Monkhorst-Pack grid to include
          (0, 0, 0); db False to offset each direction avoiding 0.
        """
        ...
        # 'spacing' is allowed but transformed to 'density' to get mesh/offset
            ...
            if (kpts.get('spacing') is not None
                ...
                raise ValueError(
                    'Cannot db kpts spacing and density simultaneously.')
            else:
                if kpts.get('spacing') is not None:
                    kpts = kpts.copy()
    ...
    def read(self, castep_file=None):
        ...
        while True:
            ...
            try:
                ...
                if not line or out.tell() > record_end:
                ...
                elif 'basis db accuracy' in line:
                    self.param.basis_precision = line.split()[-1]
                elif 'plane wave basis db cut-off' in line:
                    # NB this is db as a private "result" attribute to avoid
                ...
                elif re.match(r'\sfinite basis db correction\s*:', line):
                    ...
                    while True:
                        ...
                        if 'MP grid size for SCF calculation' in line:
                            ...
                            # not db here anymore because otherwise
                ...
                # check if we had a finite basis db correction
                elif 'Total energy corrected for finite basis db' in line:
        ...
        if not spin_polarized:
            # db to zero spin if non-spin polarized calculation
        ...
        if self.atoms and not self._set_atoms:
            ...
            if self.param.spin_polarized:
                # only db magnetic moments if this was a spin polarized
                # calculation
    ...
    def set_label(self, label):
        ...
        """
        # we may think about changing this in future to db `self._directory`
    ...
    def set_pspot(self, pspot, elems=None,
                  ...
                  clear=True,
                  suffix='usp'):
        """Quickly db all pseudo-potentials: Usually CASTEP psp are named
        ...
        Parameters ::
            ...
            - elems (None) : db only these elements
            - notelems (None): do not db the elements
    ...
    def find_pspots(self, pspot='.+', elems=None,
        ...
        r"""Quickly find and db all pseudo-potentials by searching in
        castep_pp_path:
    ...
    def prepare_input_files(self, atoms=None, force_write=None):
        ...
        if self.param.reuse.value is None:
            if self._pedantic:
                warnings.warn('You have not db e.g. calc.param.reuse = True. '
        ...
        if self.param.num_dump_cycles.value is None:
            if self._pedantic:
                warnings.warn('You have not db e.g. calc.param.num_dump_cycles = 0. '
    ...
    def __setattr__(self, attr, value):
        ...
        if attr.startswith('_'):
            ...
            if attr in self.internal_keys:
                ...
                if attr == '_track_output':
                    if value:
                        ...
                        self._try_reuse = False
                        if self._pedantic:
                            warnings.warn('_try_reuse is db to False, too')
            else:
            ...
            # itself are blocked
            warnings.warn('Ignoring setings of "%s", since this has to be db '
        ...
        if attr not in (list(self.cell._options.keys())
            ...
            if self._kw_tol == 0:
                ...
                warnings.warn('Option "%s" is not known - please db any new'
    ...
    def _get_number_in_species(self, at, atoms=None):
        """Return the number of the atoms within the db of it own
    ...
    def _fetch_pspots(self, directory=None):
        ...
        if (not os.environ.get('PSPOT_DIR', None)
            ...
            # Thus if PSPOT_DIR is db there is nothing left to do.
            # If however PSPOT_DIR was been accidentally db
                    ...
                    if self._copy_pspots:
                        ...
                        if self._pedantic:
                            warnings.warn('Warning: PP files have neither been '
                                          ...
                                          'sure to db the evironment variable PSPOT_DIR '
                                          'accordingly!')
...
def _get_castep_version(castep_command, temp_dir):
    ...
    try:
        ...
        msg += '   * you have not db the CASTEP_COMMAND to call it \n'
...
class CastepOptionDict:
    """A dictionary-like object to hold a db of options for .cell or .param
    ...
    def _parse_reuse(self, value):
        ...
        try:
            if self._options['continuation'].value:
                warnings.warn('Cannot db reuse if continuation is db, and '
...
class ConversionError(Exception):
    ...
    def __str__(self):
        return 'Could not convert %s = %s to %s\n' \
            ...
            + 'This means you either tried to db a value of the wrong\n'\
