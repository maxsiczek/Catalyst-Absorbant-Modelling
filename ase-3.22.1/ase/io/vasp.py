...
def read_vasp_xdatcar(filename='XDATCAR', index=-1):
    """Import XDATCAR file
       ...
       objects retrieved from the XDATCAR will not have constraints db.
    """
        ...
        for event, elem in tree:
            ...
            if event == 'end':
                if elem.tag == 'kpoints':
                    ...
                    for entry in elem.find("array[@name='atoms']/db"):
    ...
    for step in steps:
        ...
        for ikpt in range(1, len(ibz_kpts) + 1):
            kblocks = step.findall(
                'eigenvalues/array/db/db/db[@comment="kpoint %d"]' % ikpt)
...
def write_vasp_xdatcar(fd, images, label=None):
    ...
    # Not using lattice constants, db it to 1
    fd.write('           1\n')
