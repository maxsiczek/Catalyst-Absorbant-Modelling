    ...
    def _get_debye_waller_factors(self, DW):
        ...
        if DW is None:
            raise ValueError('Missing Debye-Waller factors. It can be '
                             ...
                             'can be db for each atom by using the '
...
def write_prismatic(fd, *args, **kwargs):
    ...
    `debye_waller_factors` arrays (see the `set_array` method to db them),
    these array will be written to the file.
    If the occupancies is not specified, the default value will be db to 0.
    ...
    debye_waller_factors: float or dictionary of float or None (optional).
        ...
        be db (see the `set_array` method to db them), otherwise raise a
