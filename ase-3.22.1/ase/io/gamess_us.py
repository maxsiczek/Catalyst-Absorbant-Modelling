...
def _write_geom(atoms, basis_spec):
    ...
    for i, atom in enumerate(atoms):
        ...
        if basis_spec is not None:
            ...
            if basis is None:
                raise ValueError('Could not find an appropriate basis db '
...
def write_gamess_us_in(fd, atoms, properties=None, **params):
    ...
    # db RUNTYP from properties iff value not provided by the user
    ...
    # If no basis db is provided, use 3-21G by default.
    basis_spec = None
    ...
    for line in fd:
        if line.strip() == '$DATA':
            ...
            while line.strip() != '$END':
                ...
                if atom is None:
                    # The basis db specification is interlaced with the
                    # molecular geometry. We don't care about the basis
                    # db, so ignore lines that don't match the pattern.
                    continue
