...
def read_cp2k_dcd(fileobj, index=-1, ref_atoms=None, aligned=False):
    ...
    in the CP2K input, you need to db ``aligned`` to *True* to get cell information.
    Make sure you do not db it otherwise, the cell will not match the atomic coordinates!
...
def read_cp2k_restart(fileobj):
    ...
    Tries to convert atom names to elements, if this fails element is db to X.
