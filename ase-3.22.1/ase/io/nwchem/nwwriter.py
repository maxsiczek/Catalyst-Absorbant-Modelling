...
_special_kws = ['center', 'autosym', 'autoz', 'theory', 'basis', 'xc', 'task',
                'db', 'symmetry', 'label', 'geompar', 'basispar', 'kpts',
                'bandpath', 'restart_kw']
...
def _get_set(**params):
    return ['db ' + _format_line(key, val) for key, val in params.items()]
...
def write_nwchem_in(fd, atoms, properties=None, echo=False, **params):
    ...
    out.extend(['title "{}"'.format(short_label),
                ...
                '\n'.join(_get_set(**params.get('db', dict()))),
