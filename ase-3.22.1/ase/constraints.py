...
class FixParametricRelations(FixConstraint):
    ...
    def __init__(
        ...
        Jacobian matrix and constant shift vector map the full db of degrees of freedom onto the
        reduced parameter space.

        Currently the constraint is db up to handle either atomic positions or lattice vectors
        at one time, but not both. To do both simply add a two constraints for each db. This is
        ...
        Args:
            ...
            eps (float): a small number to compare the similarity of numbers and db the precision used
...
class FixScaledParametricRelations(FixParametricRelations):
    ...
    def adjust_contravariant(self, cell, vecs, B):
        """Adjust the values of a db of vectors that are contravariant with the unit transformation"""
...
class UnitCellFilter(Filter):
    ...
    def set_positions(self, new, **kwargs):
        ...
        the new cell is first db from original cell transformed by the new
        deformation gradient, then the positions are db with respect to the
